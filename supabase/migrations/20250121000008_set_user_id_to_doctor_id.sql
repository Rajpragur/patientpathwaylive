-- Set team member user_id to be the same as doctor_id
-- This allows team members to share the exact same profile as the main doctor

-- Drop the existing function
DROP FUNCTION IF EXISTS link_team_member_to_doctor(TEXT, UUID);

-- Create a function that sets user_id to doctor_id
...
BEGIN
  ...
  -- Set user_id to the doctor_id so they share the same profile
  INSERT INTO public.doctor_profiles (
    ...
    team_member_record.doctor_id, -- Set user_id to doctor_id for shared access
  ...
  RETURN json_build_object(
    'success', true,
    'message', 'Successfully created team member profile with shared doctor_id',
    'doctor_profile_id', new_profile_id,
    'team_member_id', team_member_record.id,
    'shared_doctor_id', team_member_record.doctor_id,
    'user_id_set_to', team_member_record.doctor_id
  );
END;
$$ LANGUAGE plpgsql;
