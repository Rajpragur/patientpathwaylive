-- Remove the foreign key constraint issue by not updating linked_user_id
-- Just create the doctor profile and mark the invitation as used

-- Drop the existing function
DROP FUNCTION IF EXISTS link_team_member_to_doctor(TEXT, UUID);

-- Create a simplified function that avoids foreign key issues
...
BEGIN
  ...
  -- Just update the status to accepted (don't touch linked_user_id)
