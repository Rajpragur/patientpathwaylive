...
-- This script migrates existing doctor_profiles to the new clinic structure

-- Step 1: Create clinic profiles for existing doctors
...
WHERE dp.clinic_id IS NULL; -- Only migrate records not already migrated

-- Step 2: Update doctor_profiles to reference their clinic
...
-- Step 3: Create clinic members for existing doctors (as owners)
...
SELECT 
  ...
  dp.last_name,
  'owner',
  '{"leads": true, "content": true, "payments": true, "team": true}',
  'active',
  dp.created_at
FROM public.doctor_profiles dp
...
SELECT 
  ...
  END,
  CASE 
    WHEN tm.role = 'admin' THEN '{"leads": true, "content": true, "payments": false, "team": true}'
    ELSE '{"leads": true, "content": true, "payments": false, "team": false}'
  END,
  CASE 
...
WHERE public.nose_landing_pages.clinic_id IS NULL 
  AND public.nose_landing_pages.doctor_id = dp.id;
...
WHERE public.ai_landing_pages.clinic_id IS NULL 
  AND public.ai_landing_pages.doctor_id = dp.id;
